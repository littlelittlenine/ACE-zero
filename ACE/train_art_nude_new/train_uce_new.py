                ...
                with torch.no_grad():
                    for layer_number, layer in enumerate(projection_matrices):
                        ...
                        if technique == 'tensor':
                            ...
                            u = o_embs
                            u = u / u.norm()
                            # 每一轮都会存储一下，每个layer，虽然浪费时间，但是没问题
                            torch.save(o_embs, '/mnt/bn/intern-disk/mlx/users/wrp/uce_nullspace/model_save/erase_nude/all_forward_reverse/o_embs_{}.pt'.format(layer_number))
                            new_embs = layer(new_emb).detach()
                            new_emb_proj = (u*new_embs).sum()
                            # target = project_to_null_space(new_embs, projection, o_embs)
                            target = new_embs - (new_emb_proj)*u
            ...
            for cnt, t in enumerate(zip(old_texts, new_texts)):
                ...
                with torch.no_grad():
                    for layer_number, layer in enumerate(projection_matrices):
                        ...
                        o_embs = layer(old_emb).detach()
                        if technique == 'tensor':
                            o_embs = layer(old_emb).detach() # (75,320)
                            # u = o_embs
                            # u = u / u.norm()
                            new_embs = layer(new_emb).detach()
                            # new_emb_proj = (u*new_embs).sum()
                            ...
                            o_embs = torch.load(f'/mnt/bn/intern-disk/mlx/users/wrp/uce_nullspace/model_save/erase_nude/all_forward_reverse/o_embs_{layer_number}.pt')
                            u = old_emb
                            u = u / u.norm()
                            context_proj = (u*context).sum()
                            context = context - (context_proj)*u
                            # target = new_embs - (new_emb_proj)*u
                            values.append(o_embs.detach()) 
                        else:
                            values.append(layer(new_emb).detach())
                #  先乘法再求和和先求和再乘的区别
                context_vector = context.reshape(context.shape[0], context.shape[1], 1) # (76, 768, 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_save_path', help='Path to save the model', type=str, required=True)
    parser.add_argument('--concepts_save_path', help='Path to save the concepts', type=str, required=True)
    ...
    # 保存反向编辑的模型和概念
    ldm_stable = edit_model_reverse(ldm_stable=ldm_stable, old_text_=old_texts, new_text_=new_texts, add=False, retain_text_=retain_texts, lamb=0.1, erase_scale=erase_scale, preserve_scale=preserve_scale, technique=technique)
