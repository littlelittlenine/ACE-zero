...
if __name__ == '__main__':
    ...
    # ldm_stable.unet.load_state_dict(torch.load(model_path, map_location=device))
    with open('/mlx/mlx_devbox/users/wangruipeng/playground/paper_run/uce_nullspace/unified-concept-editing-main/train_nude_new/SD21_uce.txt', 'a') as time_log:  # 'w'模式会创建一个新文件
        batch_size = 100   
        for i in tqdm(range(0, len(old_texts), batch_size)): 
            ...
            model_save_path_with_index = args.model_save_path.replace('.pt', f'_batch_{batch_index}.pt')  
            concepts_save_path_with_index = args.concepts_save_path.replace('.txt', f'_batch_{batch_index}.txt')  
            ...
            # 更新保留文本
            retain_texts += old_text  # 添加当前处理的 old_text 到 retain_texts