...
def alpha_edit_2(ldm_stable, old_text_, new_text_, retain_text_, add=False, layers_to_edit=None, lamb=0.1, erase_scale=0.1, preserve_scale=0.1, with_to_k=True, technique='tensor', P=None, lamda=10, cache_c = None):
    ...
    P2 = P.clone()
    for layer_num in range(len(projection_matrices)):
        # 需要得到这个矩阵输出端的投影矩阵：是不是可以male和female组成的矩阵，这里有两个方式male和female，这个时候male 和 female 需要包含male doctor和female doctor组成的样本集比较好还有其他职业，也就是male和female在不同的语境下，然后将doctor投影到这个前面的嵌入的零空间中，第二种就是使得doctor的输出编码和前面的嵌入组形成的空间轴的夹角都相同
        ...
        with torch.no_grad():  # 计算图控制
            ...
            for cnt, (old_text, new_text) in enumerate(zip(old_texts, new_texts)):
                ...
                old_emb = old_emb[idx_old-1:idx_old] # （4，768）
                new_emb = new_emb[idx_new-1:idx_new] # （4，768）
                # else:
                # # old_emb = old_emb[final_token_idx:len(old_emb)-max(0,farthest-final_token_idx)] # （76，768）
...
if __name__ == '__main__':
    ...
    # bias的时候这里需要重新设置
        ...
        # new_texts = ["Make sure the gender ratio is evenly distributed and avoid reinforcing gender bias in professions to achieve a one to one ratio of male and female " + item.lower() + item.lower() for item in old_texts]
