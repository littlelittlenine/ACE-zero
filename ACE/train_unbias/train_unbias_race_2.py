...
# 新的方法，就是每次寻找最大的和第二大的比如是latino doctor和european doctor，向european doctor编辑，使得这两个的比例相同，然后编辑再加上剩下的编辑，注意投影矩阵P @ W @ P1要求P满足是latino doctor和european doctor的正交投影矩阵
import numpy as np
import torch
import random
import pandas as pd
...
def alpha_edit_3(ldm_stable, old_text_, new_text_, retain_text_, add=False, layers_to_edit=None, lamb=0.1, erase_scale=0.1, preserve_scale=0.1, with_to_k=True, technique='tensor', P=None, lamda=10, cache_c = None, P_outs = None):
    ...
    for layer_num in range(len(projection_matrices)):
        ...
        with torch.no_grad():  # 计算图控制
            ...
            # (320,768) @ (768,768) = (320,768)
            upd_matrix = torch.linalg.solve(
                result1.transpose(0, 1), # 这个转置是有用的，
                result2.transpose(0, 1)
            )
            # 权重：（320，320）*（320，1）*（1，320）
            # 更新投影矩阵权重
...
def get_project_output(ldm_stable, preserve_concepts, percentage_of_smallest_singular=0.01, batch_size=16, with_to_k=True):
    ...
    return P_outs
# # 一种是直接把male 和 female相关的都输入，找到投影矩阵，一种是分别对male和female求投影矩阵,concept是列表形式了
def K_means_output(ldm_stable, concept, with_to_k=True, i=0):
    ...
    for layer_num in range(len(projection_matrices)):
        ...
        if i!=0:
            ...
            # 清理每个 prompt，去掉不必要的引号和多余的空格
            print(f"layer_num: {layer_num}, concept: {concept}")
            ...
            with torch.no_grad():  # 不计算梯度以节省显存
                ...
                text_embeddings = text_embeddings.detach()  # 从计算图中剥离张量 # (1,77,768)
                output_embeddings = projection_matrices[layer_num](text_embeddings).detach()       
                output_embeddings = output_embeddings[:,idx-1,:] # 应该是(X,320)
        ...
        P_outs.append(P_out)        
    return P_outs
def K_means_output_nullspace(ldm_stable, concept, with_to_k=True, num_smallest_singular=100, i=0):
    ### 收集所有交叉注意力模块 ###
    sub_nets = ldm_stable.unet.named_children()
    ca_layers = []
    P_outs = []
        ...
        if i!=0:
            ...
            # if i!=0:
                ...
                with open(log_file_path, 'a') as log_file:
                    total_embeddings_size = total_embeddings.size()
                    log_file.write(f"total_embeddings.size:{total_embeddings_size}\n")
                    # print('total_embeddings.size:',total_embeddings.size())
...
if __name__ == '__main__':
    ...
    log_file_path = '/mlx_devbox/users/wangruipeng/playground/paper_run/uce_nullspace/unified-concept-editing-main/train_unbias/log_file_race.txt'  # 替换为您希望保存日志文件的路径
    # 打开文件以追加模式
    with open(log_file_path, 'a') as log_file:
        for i in tqdm(range(0, len(old_texts), batch_size)):  # 其实i就1个doctor
            ...
            for j in range(4):  # 外层循环
                ...
                counts_indian, counts_asian, counts_african, counts_european, counts_latino = CLIP_classification_race(
                    im_path=img_path, 
                    attributes='indian, asian, african, european, latino', 
                    prompts_path='/mlx_devbox/users/wangruipeng/playground/paper_run/uce_nullspace/unified-concept-editing-main/data/profession/Analyst/analyst_origin.csv', 
                    save_path=None, 
                    from_case=0, 
                    till_case=10000
                )
                ...
                name_second = list(counts_dict.keys())[j]
                counts_second = counts_dict[name_second]
                name_first = 'latino doctor'
                counts_first = counts_latino

                min_difference = min(min_difference, abs(counts_second - counts_first))
                if j%4==0:
                    preserve_output = []
                skip_outer_loop = False  # 标志，指示是否跳过外层循环
                current_sum_of_abs_diff = 200
                for num_smallest_singular_j in range(10, 270, 20):
                    P_outs = K_means_output_nullspace(ldm_stable, preserve_output, num_smallest_singular_j, j % 4)

                    for num_smallest_singular in range(200, 670, 10):  # 内层循环
                        print(f'num_smallest_singular:{num_smallest_singular}, j:{j}, index:{index}')
                        P = get_project_input_3(ldm_stable, coco_path, num_smallest_singular=num_smallest_singular, batch_size=16)
                        log_file.write(f"new_text old_text: {[old_text[0], name_second]}\n")
                        log_file.flush()
                        name = [name_second]
                        model_save_path_with_index = args.model_save_path.replace('.pt', f'_race_batch_{batch_index}_j_{j}_lamb_{lamb}_num_smallest_{num_smallest_singular}_{num_smallest_singular_j}.pt')
                        concepts_save_path_with_index = args.concepts_save_path.replace('.txt', f'_race_batch_{batch_index}_j_{j}_lamb_{lamb}_num_smallest_{num_smallest_singular}_{num_smallest_singular_j}.txt')

                        ldm_stable, cache_c = alpha_edit_2(
                        ...
                        counts_second = counts_dict[name_second]
                        counts_first = counts_dict[name_first]

                        preserve_output.append(name_second)
                        preserve_output.append(name_first)
                        num = []
                        preserve_output = list({item.strip() for item in preserve_output})  # 去重的同时去除空格
                        print(f"preserve_output:{preserve_output}")
                        log_file.write(f"preserve_output:{preserve_output}\n")
                        log_file.flush()
                        for i in range(len(preserve_output)):
                            num.append(counts_dict[preserve_output[i]])
                        average = sum(num) / len(num)
                        sum_of_abs_diff = sum(abs(x - average) for x in num)
                        print(f'min_difference:{min_difference}, {counts_second - counts_first}')
                        if sum_of_abs_diff <= len(num)*2:
                            log_file.write("find best one\n")
                            log_file.flush()
                            last_saved_model_state = ldm_stable.unet.state_dict()
                            ...
                            skip_outer_loop = True  # 设置标志以跳过外层循环
                            break  # 跳出内层循环  
                        elif sum_of_abs_diff <= current_sum_of_abs_diff:                          
                        # elif counts_second == counts_first or abs(counts_second - counts_first) <=2:
                            log_file.write("find near best one\n")
                            log_file.flush()
                            current_sum_of_abs_diff = sum_of_abs_diff
                        #     min_difference = abs(counts_second - counts_first)
                        ...
                        #     # break  # 跳出内层循环
                            ldm_stable.unet.load_state_dict(last_saved_model_state)
                            cache_c = cache_c_last
                        # elif abs(counts_second - counts_first) < min_difference and (counts_second - counts_first) < 2:
                        #     log_file.write("continue find near one\n")
                        #     min_difference = abs(counts_second - counts_first)
                        #     get_saved_model_state = ldm_stable.unet.state_dict()
                        #     ldm_stable.unet.load_state_dict(last_saved_model_state)
                        #     cache_c = cache_c_last

                        elif num_smallest_singular == 650:
                            log_file.write("end\n")
                            log_file.flush()
                            cache_c = cache_c_last  # cache_c永远是个定值
                            last_saved_model_state = get_saved_model_state
                            ldm_stable.unet.load_state_dict(get_saved_model_state)
                            torch.save(get_saved_model_state, model_save_path_with_index)
                            with open(concepts_save_path_with_index, 'w') as fp:
                                json.dump(concepts, fp)
                            skip_outer_loop = True  # 设置标志以跳过外层循环
                            break  # 跳出内层循环
                            ...
                            ldm_stable.unet.load_state_dict(last_saved_model_state)
                            cache_c = cache_c_last 
                        log_file.write(f'counts:{counts_second} {counts_first}\n')
                        print(f'counts:{counts_second} {counts_first}\n')
                        log_file.flush()
                    if skip_outer_loop:  # 如果设置了标志，跳出外层循环
                        break
# 为什么会一直编辑为indian doctor呢