                ...
                with torch.no_grad():
                    for layer_number, layer in enumerate(projection_matrices):
                        ...
                        if technique == 'tensor':
                            ...
                            values.append(layer(new_emb).detach())
                #  先乘法再求和和先求和再乘的区别
                context_vector = context.reshape(context.shape[0], context.shape[1], 1) # (4, 768, 1)
                context_vector_T = context.reshape(context.shape[0], 1, context.shape[1]) # (4, 1, 768)
                value_vector = values[layer_num].reshape(values[layer_num].shape[0], values[layer_num].shape[1], 1) # (4, 320, 1)
    ...
    for layer_num in range(len(projection_matrices)):
        ...
        with torch.no_grad():
            ...
            for cnt, t in enumerate(zip(old_texts, new_texts)):
                ...
                ### 寻找doctor索引，同时得到它后面的第一个<EOS>索引,doctor的索引是5547：idx和idx+1
                print("start to find doctor")
                print(text_input)
                print(text_input.input_ids[0])
                print(text_input.input_ids[0][0])
                idx_old = text_input.input_ids[0][0].tolist().index(ldm_stable.tokenizer.get_command_id(5547))
                idx_new = text_input.input_ids[0][1].tolist().index(ldm_stable.tokenizer.get_command_id(5547))
                old_emb = text_embeddings[0] # (77, 768)
                # old_emb = old_emb[final_token_idx:len(old_emb)-max(0,farthest-final_token_idx)] # （76，768）
                old_emb = old_emb[idx_old:idx_old + 2] # （76，768）
                new_emb = text_embeddings[1] # (77, 768)
                # new_emb = new_emb[final_token_idx_new:len(new_emb)-max(0,farthest-final_token_idx_new)] # （76，768）
                new_emb = new_emb[idx_new:idx_new + 2] # （76，768）
...
if __name__ == '__main__':
    ...
    # bias的时候这里需要重新设置
        ...
        new_texts = ["gender balance, " + item for item in old_texts]
