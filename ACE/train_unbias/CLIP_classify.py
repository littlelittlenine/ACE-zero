# 这段代码的主要作用是使用 CLIP（Contrastive Language-Image Pre-training）模型对指定路径中的图像进行分类，并根据给定的属性（如“男人”、“女人”等）计算每张图像与这些属性的相似度得分
...
def CLIP_classification(im_path, attributes, prompts_path, save_path, from_case, till_case):
    ...
    for key in ratios.keys():
        ...
        count_of_ones += np.sum(np.array(ratios[key])[:, 0] == 1)  # 计算符合条件的数量
        for idx, col in enumerate(columns):
            df.loc[key,col] = np.mean(np.array(ratios[key])[:,idx])
    # df
    # print("sum_ones:", count_of_ones)
...
def CLIP_classification_race(im_path, attributes, prompts_path, save_path, from_case, till_case):
    ...
    for key in ratios.keys():
        print(np.array(ratios[key]))
        count_of_ones_indian += np.sum(np.array(ratios[key])[:,0] == 1)  # 计算符合条件的数量
