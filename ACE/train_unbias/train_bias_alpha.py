...
def alpha_edit(ldm_stable, old_text_, new_text_, retain_text_, add=False, layers_to_edit=None, lamb=0.1, erase_scale=0.1, preserve_scale=0.1, with_to_k=True, technique='tensor', P=None, lamda=10):
    ...
    return ldm_stable
def alpha_edit_2(ldm_stable, old_text_, new_text_, retain_text_, add=False, layers_to_edit=None, lamb=0.1, erase_scale=0.1, preserve_scale=0.1, with_to_k=True, technique='tensor', P=None, lamda=10):
    ### 收集所有交叉注意力模块
    sub_nets = ldm_stable.unet.named_children()
    ca_layers = []
...
if __name__ == '__main__':
    ...
    if 'professions' in concepts[0]: # 去偏见200个职业professions2000
    ...
    # bias的时候这里需要重新设置
        ...
        new_texts = ["Make sure the gender ratio is evenly distributed and Reinforcing gender bias in professions to achieve a one to one ratio of male and female doctors " + item for item in old_texts]
    ...
    ldm_stable = alpha_edit_2(ldm_stable= ldm_stable, old_text_= old_texts, new_text_=new_texts, add=False, retain_text_= retain_texts, lamb=0.1, erase_scale = erase_scale, preserve_scale = preserve_scale,  technique=technique, P=P)
    torch.save(ldm_stable.unet.state_dict(), args.model_save_path)
    # 保存概念
    with open(args.concepts_save_path, 'w') as fp:
