...
def edit_model(ldm_stable, old_text_, new_text_, retain_text_, add=False, layers_to_edit=None, lamb=0.1, erase_scale = 0.1, preserve_scale = 0.1, with_to_k=True, technique='tensor', projection_path='/mnt/bn/intern-disk/mlx/users/wrp/uce_nullspace/model_save/nullspace_project/output_2000_100/projection_matrix_{}.pt'):
    ...
    return ldm_stable
# 这里输入的emb的形状是(N,768) 或者(N,320),返回的是正交投影矩阵
# 这个如何处理
def get_projection(emb):
    I = torch.eye(emb.shape[1], device = (emb.device))
    return I - emb.T @ torch.inverse(emb @ emb.T) @ emb
# def get_proj_matrix_torch(embeddings):
#     # 注意：TruncatedSVD 是 scikit-learn 的一个组件，它不是 PyTorch 的部分。
#     # 如果你想要在 PyTorch 中实现 SVD，你需要使用 torch.svd。
#     # 但在这里，我们将使用 TruncatedSVD 来拟合并转换数据，然后将其转换为 PyTorch 张量。
    
#     tSVD = TruncatedSVD(n_components=len(embeddings))
#     embeddings_ = tSVD.fit_transform(embeddings)
#     basis = torch.tensor(tSVD.components_.T, dtype=torch.float32)

#     # orthogonal projection
#     proj = torch.linalg.inv(torch.matmul(basis.t(), basis))
#     proj = torch.matmul(basis, proj)
#     proj = torch.matmul(proj, basis.t())
#     proj = torch.eye(proj.shape[0]) - proj
#     return proj
def normalize_vector(v):
    """归一化向量"""
    norm = torch.norm(v, dim=1, keepdim=True)  # 沿第1维度计算范数
    return v / (norm + 1e-8)  # 防止除零

def compute_plane_normal(embeddings1, embeddings2):
    """
    计算平面的法向量，使得投影到 embeddings1 和 embeddings2 方向上的模值相同。
    """
    # 归一化 embeddings1 和 embeddings2
    e1 = normalize_vector(embeddings1)  # 形状: (1, 768)
    e2 = normalize_vector(embeddings2)  # 形状: (1, 768)
    
    # 计算法向量
    n = e1 - e2  # 形状: (1, 768)
    n = normalize_vector(n)  # 归一化
    return n

def build_projection_matrix(n):
    """
    根据平面的法向量构建投影矩阵
    """
    I = torch.eye(n.size(1)).to(n.device)  # 单位矩阵，形状: (768, 768)
    n = n.reshape(-1, 1)  # 将法向量转换为列向量，形状: (768, 1)
    
    # 使用 torch.matmul 进行矩阵乘法
    P = I - torch.matmul(n, n.T) / torch.matmul(n.T, n)  # 投影矩阵，形状: (768, 768)
    
    return P

def project_vector_to_plane(x, P):
    """
    将向量 x 投影到平面上
    """
    return torch.mm(x, P)  # 矩阵乘法，形状: (1, 768)

def project_to_common_plane(embeddings1, embeddings2, input_vector):
    """
    将输入向量投影到平面，使得投影到 embeddings1 和 embeddings2 方向上的模值相同。
    """
    # 1. 计算平面的法向量
    n = compute_plane_normal(embeddings1, embeddings2)
    
    # 2. 构建投影矩阵
    P = build_projection_matrix(n)
    # # 3. 将输入向量投影到平面上
    # projected_vector = project_vector_to_plane(input_vector, P)
    return P
    # # 3. 将输入向量投影到平面上
    # projected_vector = project_vector_to_plane(input_vector, P)
    
    # # 4. 将 embeddings1 和 embeddings2 投影到平面上
    # projected_emb1 = project_vector_to_plane(embeddings1, P)
    # projected_emb2 = project_vector_to_plane(embeddings2, P)
    
    # return projected_vector, projected_emb1, projected_emb2, n, P


def get_proj_matrix(embeddings):
    # 打印 embeddings 的大小
    print("embeddings:", embeddings.size())  # （2，768）这里是male和female两个方向
    
    # 保存原始设备
    original_device = embeddings.device
    
    # 确保 embeddings 在 CPU 上进行处理
    ...
    # 进行截断 SVD
    tSVD = TruncatedSVD(n_components=len(embeddings_np))
    embeddings_ = tSVD.fit_transform(embeddings_np)
    ...
    # 使用 torch.from_numpy() 创建 PyTorch 张量，并将其移动到原始设备上
    return torch.from_numpy(proj).to(original_device)

def get_projection(embeddings):
    # 计算单位向量
    norm_embeddings = embeddings / embeddings.norm(dim=-1, keepdim=True)
    
    # 计算角平分线方向
    angle_bisector = (norm_embeddings[0] + norm_embeddings[1]) / 2
    angle_bisector = angle_bisector / angle_bisector.norm()  # 归一化
    
    # 生成新的投影矩阵
    proj_matrix = get_proj_matrix(angle_bisector.unsqueeze(0))  # 生成新的投影矩阵
    return proj_matrix

def get_P(ldm_stable, concept, prompt1, prompt2, with_to_k=True):
    sub_nets = ldm_stable.unet.named_children()
    ca_layers = []
    ...
    # 获取文本的嵌入
    text_input_1 = ldm_stable.tokenizer(
        prompt1,
        padding="max_length",
        max_length=ldm_stable.tokenizer.model_max_length,
        truncation=True,
    ...
    text_input_2 = ldm_stable.tokenizer(
        prompt2,
        padding="max_length",
        max_length=ldm_stable.tokenizer.model_max_length,
        truncation=True,
    ...
    concept_input = ldm_stable.tokenizer(
        concept,
        padding="max_length",
        max_length=ldm_stable.tokenizer.model_max_length,
        truncation=True,
    ...
    # 确保所有输入都在指定设备上
    idx_prompt1 = text_input_1.input_ids[0].tolist().index(49407)
    idx_prompt2 = text_input_1.input_ids[0].tolist().index(49407)
    idx_concept = text_input_1.input_ids[0].tolist().index(49407)
    concept_emb = ldm_stable.text_encoder(concept_input.input_ids.to(ldm_stable.device))[0][:, idx_concept, :]
    print("concept_emb.size:", concept_emb.size())
    P0 = get_projection(concept_emb)
    
    zi = ldm_stable.text_encoder(text_input_1.input_ids.to(ldm_stable.device))[0][:, idx_prompt1, :]
    zj = ldm_stable.text_encoder(text_input_2.input_ids.to(ldm_stable.device))[0][:, idx_prompt2, :]
    
    # 获取输出端的矩阵
    P_outs = []
    print("P0的类型:", type(P0))
    print("device:", P0.device)  
    eye_matrix = torch.eye(P0.shape[1], device=P0.device).type(torch.float32)
    P0 = P0.type(torch.float32)
    # 计算差矩阵并确保其类型为 Float
    diff_matrix = (zi - zj).T @ (zi - zj)
    diff_matrix = diff_matrix.type(torch.float32)
    # 计算投影矩阵，确保所有张量类型一致
    P_in = P0 @ torch.inverse(eye_matrix + diff_matrix)
    P_in = project_to_common_plane(zi, zj, concept_emb)
    for layer_num in range(len(projection_matrices)):
        with torch.no_grad():
            output_embeddings_concept = projection_matrices[layer_num](concept_emb).detach()
            output_embeddings_zi = projection_matrices[layer_num](zi).detach()
            output_embeddings_zj = projection_matrices[layer_num](zj).detach()
            # 计算投影矩阵
            P0 = get_projection(output_embeddings_concept).type(torch.float32)
            print("device:", P0.device)
            # 计算差矩阵并确保其类型为 Float
            eye_matrix = torch.eye(P0.shape[1], device=P0.device).type(torch.float32)
            diff_matrix = (output_embeddings_zi - output_embeddings_zj).T @ (output_embeddings_zi - output_embeddings_zj)
            diff_matrix = diff_matrix.type(torch.float32)  
            P_out = P0 @ torch.inverse(eye_matrix + diff_matrix)     
            P_outs.append(P_out)

    print("len(P_outs):", len(P_outs))
    return P_in, P_outs
def get_P_2(ldm_stable, concept, prompt1, prompt2, with_to_k=True):
    sub_nets = ldm_stable.unet.named_children()
    ca_layers = []
    ...
    concept_emb = ldm_stable.text_encoder(concept_input.input_ids.to(ldm_stable.device))[0][:, idx_concept, :]
    print("concept_emb.size:", concept_emb.size())
    P0 = get_proj_matrix(concept_emb)
    ...
    for layer_num in range(len(projection_matrices)):
        with torch.no_grad():
            ...
            # 计算投影矩阵
            # 计算两个嵌入之间的差异向量
            diff_vector = output_embeddings_zi - output_embeddings_zj

            # 计算差异向量的单位向量，这将是中垂面的法向量
            normal_vector = diff_vector / torch.norm(diff_vector)

            # 构造投影矩阵 P_out，它将任何向量投影到由 normal_vector 定义的平面上
            # 投影矩阵 P_out = I - normal_vector * normal_vector^T
            # 其中 I 是单位矩阵，normal_vector^T 是 normal_vector 的转置

            identity_matrix = torch.eye(normal_vector.shape[0], device=normal_vector.device)
            normal_vector_outer_product = torch.outer(normal_vector, normal_vector)
            P_out = identity_matrix - normal_vector_outer_product                 
            P_outs.append(P_out)
    print("len(P_outs):", len(P_outs))
    return P_in, P_outs
...
def proj(emb1, P1, P2, max_iterations=1000, learning_rate=1e-2):
    torch.set_grad_enabled(True)
    emb1 = emb1.float()
    new_embs = emb1.clone().requires_grad_(True)  # 确保新嵌入需要梯度
    optimizer = torch.optim.Adam([new_embs], lr=learning_rate)

    for iteration in range(max_iterations):
        optimizer.zero_grad()
        
        # 投影计算
        proj_emb1_P1 = P1 @ new_embs.T
        proj_emb1_P2 = P2 @ new_embs.T
        
        # 计算模值（确保保持梯度）
        norm_P1 = torch.norm(proj_emb1_P1, dim=1)  # 使用 dim=1 计算每行的模
        norm_P2 = torch.norm(proj_emb1_P2, dim=1)  # 使用 dim=1 计算每行的模
        
        # 损失计算
        loss_projection = F.mse_loss(norm_P1, norm_P2)
        loss_distance = F.mse_loss(new_embs, emb1)

        # 确保损失是标量
        total_loss = 5 * loss_projection + loss_distance
        
        # 打印信息
        if iteration % 100 == 0:
            print(f"Iteration {iteration}, Total Loss: {total_loss.item()}, "
                  f"Norms: {norm_P1.mean().item()}, {norm_P2.mean().item()}")

        total_loss.backward()  # 计算梯度
        optimizer.step()

    return new_embs.detach()


def alpha_edit_2(ldm_stable, old_text_, new_text_, retain_text_, add=False, layers_to_edit=None, lamb=0.1, erase_scale=0.1, preserve_scale=0.1, with_to_k=True, technique='tensor', P=None, lamda=10, cache_c = None, P_outs = None, P_in = None):
    ### 收集所有交叉注意力模块
    sub_nets = ldm_stable.unet.named_children()
    ca_layers = []
    ...
    for layer_num in range(len(projection_matrices)):
        # 我们要寻找的是和Wv矩阵对应的Wk矩阵,以及和Wk矩阵对应的Wv矩阵
        # P_out1 = P_outs1[layer_num]
        # P_out2 = P_outs2[layer_num]
        print("layers_to_edit", layers_to_edit) 
        if (layers_to_edit is not None) and (layer_num not in layers_to_edit):
            continue
        print(f'Editing layer {layer_num}')
        with torch.no_grad():  # 计算图控制
            ...
            for cnt, (old_text, new_text) in enumerate(zip(old_texts, new_texts)):
                ...
                context_vector = context.reshape(context.shape[0], context.shape[1], 1) # (2, 768, 1)
                context_vector_T = context.reshape(context.shape[0], 1, context.shape[1]) # (2, 1, 768)
                value_vector = (context @ P_in @ W_old.T).detach() # (2, 320, 1)
                # value_vector = value_vector @ P_out.T
                # value_vector = proj(value_vector, P_out1, P_out2)
...
def get_project_output(ldm_stable, preserve_concepts, singular_value_threshold=2e-2, batch_size=16, with_to_k=True):
    ### 收集所有交叉注意力模块 ###
    sub_nets = ldm_stable.unet.named_children()
    ca_layers = []
    ...
    for layer_num in range(len(projection_matrices)):
        ...
        # 选择小于阈值的奇异值的索引
        smallest_indices = (S < singular_value_threshold).nonzero(as_tuple=True)[0]
        
        if smallest_indices.numel() == 0:
            print(f"No singular values below the threshold {singular_value_threshold}.")
            print(f"Layer {layer_num} will not be clipped.")
            continue  # 如果没有奇异值小于阈值，跳过该层
...
if __name__ == '__main__':
    ...
    if 'professions5' in concepts[0]: # 去偏见200个职业professions2000
        ...
        output_concepts = ['male', 'man','female', 'woman']
    ...
    # bias的时候这里需要重新设置
        ...
        for item in old_texts:
            ...
            new_texts.append(f"Generate an equal number of male {item_singular} and female {item_singular}")
    ...
    if preserve_concepts is None:
        if concept_type == 'art':
            ...
            # P_in = get_P(ldm_stable, )
            # P_outs1 = get_project_output(ldm_stable, output_male_concepts, singular_value_threshold=2e-2, batch_size=1)
            # P_outs2 = get_project_output(ldm_stable, output_female_concepts, singular_value_threshold=2e-2, batch_size=1)
            preserve_concepts = []
        else:
            preserve_concepts = []
    # 先不保留知识，UCE
    ...
    for i in tqdm(range(0, len(old_texts), batch_size)): # 方便修改！！！！！！！！！！！！！！！！！
        ...
        prompt1 = "male " + old_text[0]
        prompt2 = "female " + old_text[0]
        # 打印查看效果
        print([old_text, prompt1, prompt2])
        P_in, P_outs = get_P(ldm_stable, output_concepts, prompt1, prompt2)
        # 使用批次索引更新模型保存路径
        batch_index = i // batch_size  # 计算批次索引
        model_save_path_with_index = args.model_save_path.replace('.pt', f'_bias_batch_{batch_index}_lamb_{lamb}_num_smallest_{args.num_smallest_singular}.pt')  # 添加lamb到文件名
        concepts_save_path_with_index = args.concepts_save_path.replace('.txt', f'_bias_batch_{batch_index}_lamb_{lamb}_num_smallest_{args.num_smallest_singular}.txt')  # 添加lamb到文件名
        
        ldm_stable, cache_c = alpha_edit_2(ldm_stable=ldm_stable, old_text_=old_text, new_text_=new_text, add=False, retain_text_=retain_texts, lamb=lamb, erase_scale=erase_scale, preserve_scale=preserve_scale, technique=technique, lamda=lamda, P=P, cache_c=cache_c, P_outs=P_outs, P_in=P_in)
