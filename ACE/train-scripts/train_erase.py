...
# 我需要判断一下csv里面的token是不是和nudity的相似程度


def get_similar_token(ldm_stable, old_text_, new_text_, retain_text_, add=False, layers_to_edit=None, lamb=0.1, erase_scale = 0.1, preserve_scale = 0.1, with_to_k=True, technique='tensor'):
    ### collect all the cross attns modules
    # 获取所有交叉注意力模块
    # 定义了一个名为 max_bias_diff 的变量，并将其赋值为 0.05，控制或限制某些操作中的偏差差异
    token = 'nudity'
